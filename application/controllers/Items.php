...
function dokumen(){
   ...
   public function edit(){
    ...
    if($this->form_validation->run() !== FALSE){
        ...
        $itemPIC = set_value('itemPIC');
        $itemNUP = set_value('itemNUP');
        $itemTahun = set_value('itemTahun');
        $itemKondisi = set_value('itemKondisi');
        $itemUnit = set_value('itemUnit');
        $itemCode = $this->input->post('itemCode', TRUE);
        //update item in db
        $updated = $this->item->edit($itemId,$itemName,$itemKategori,$barCode,$itemLokasi,$itemDesc,$itemPIC,$itemNUP,$itemTahun,$itemKondisi,$itemUnit,$itemCode);
